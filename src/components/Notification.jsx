...
export const Failure = ({ message }) => {
  const hideBar = () => {
    const successBar = document.getElementById('failureBar')
    successBar.classList.add('hidden')
  }

  setTimeout(hideBar, 2000);
  return (
    <div id='failureBar' className='absolute right-10 bottom-10 z-50 w-[300px] h-[50px] bg-[#F84F31] flex justify-between items-center px-5 text-white font-semibold text-lg'>
      {message}
    </div>
  )
}