...
export const SmallInput = ({ type, placeholder, name, state = '', setState = () => { } }) => {
    return (
        <>
            <input className='input px-3 h-[35px] flex items-center w-[30%] focus:outline-2 focus:outline-bgGreen text-black group font-semibold z-0'
                type={type}
    ...
    return (
        <>
            <div className='relative z-0'>
                ...
                <input className='input px-3 h-[35px] w-[100%] focus:outline-2 focus:outline-bgGreen text-black group font-semibold' type='date' value={state[name]} onChange={(e) => setState({ ...state, [name]: e.target.value })} />
            </div>
        </>
    )
}
